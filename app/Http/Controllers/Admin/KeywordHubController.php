...
class KeywordHubController extends Controller
{
    ...
    {
        // 후보 카운팅은 120만 행 집계(유형×상태 조인 실측 ~7초)라 캐시 — 페이지 로딩이 이것 때문에 수 초씩 걸렸다.
        // flexible(SWR): 60초 지나면 낡은 값을 즉시 내주고 응답 뒤 백그라운드 재계산 — 방문자는 항상 즉시 로드.
        // 진행 실시간 표시는 가벼운 autoStatus 폴링이 담당하므로 대시보드 집계는 분 단위 지연 허용.
        // ⚠️ database 캐시는 객체(Collection)가 왕복에서 __PHP_Incomplete_Class 로 깨진다(실측 500) — 순수 배열만 저장.
        $agg = Cache::flexible('hub:admin-agg', [60, 1800], fn () => [
