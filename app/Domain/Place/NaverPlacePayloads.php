...
JSON;
        ...
        $restaurants = <<<'JSON'
[{"operationName":"getRestaurantsPcmap","variables":{"input":{"query":"{#query}","start":{#start},"display":50,"isCurrentLocationSearch":null,"deviceType":"pcmap","isPcmap":true}},"query":"query getRestaurantsPcmap($input: PlaceListInput) {  restaurants: placeList(input: $input) {    businesses {      total      items {        id        name        category        businessCategory        x        y        visitorReviewCount        blogCafeReviewCount        bookingReviewCount        visitorReviewScore        tags        saveCount        imageCount        newOpening        talktalkUrl        posInfo {          isPOS          __typename        }        __typename      }      __typename    }    __typename  }}"}]
JSON;
