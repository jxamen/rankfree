...
| `search.shopping.naver.com/search/all` | 307 → 로그인 | **405 + 캡차** — ⚠️ **자동화 브라우저 한정**. 실제 크롬에서는 정상 동작한다(시장분석이 이 경로로 운영 중) |
...
**⚠️ 측정 함정(2026-08-03):** Playwright 등 자동화 브라우저로 `search/all`·`ns/search` 를 재면 307/418/캡차가 나온다.
**이건 자동화 탐지지 URL 이 죽은 게 아니다** — 실제 크롬에서는 시장분석이 같은 경로(`collectShopping` → 백그라운드 탭)로
지금도 정상 동작한다. 통합검색 페이지에서 시장분석을 열면 바로 이 경로를 탄다(`collectViaShoppingTab`).
**수집 가부는 자동화 브라우저로 판단하지 말 것.**

**한계:** 순위 범위는 확장이 수집한 페이지 수까지(1페이지=80위). 시장분석은 최대 5페이지로 검증됐지만
순위추적은 13페이지까지 간다 — 깊은 페이지일수록 간격을 벌린다(400ms → 최대 1.2초).
**부분 수집은 미노출로 확정하지 않는다**(요청 깊이의 90% 미만이면 재큐 — 700위 상품이 '순위권 밖' 으로 남으면 그래프가 거짓이 된다).
