...
- **유입 키워드 자동 추천(2026-08-27)** — [PlaceKeywordSuggester](../app/Domain/Place/PlaceKeywordSuggester.php). 상호명·지역(시/구/동, 동은 '초량동→초량' 로 접미 제거)·업종을 섞어 후보를 만들고(`{지역}{업종}`·`{지역}{업종}추천`·`{지역}{업종}잘하는곳`·`{상호}`·`{지역}{상호}` 등, 상한 14개), **키워드마다 실제 플레이스 순위를 조회해 상위 50위 안에 잡히는 것만** 채택한다. 결과는 `{keyword, rank}` 로 **순위와 함께** 돌려주고 좋은 순으로 정렬한다. 화면 [키워드 자동 추천] 버튼(`POST admin/orders/{order}/boosting-shop/keywords`)이 호출하며, 지금 입력돼 있는 키워드도 함께 검사해 순위에 없으면 제외 목록으로 알려준다.
  - 순위 조회는 [PlaceRankChecker](../app/Domain/Place/PlaceRankChecker.php)(플레이스 순위추적과 같은 엔진 · nCaptcha 토큰 사용). 추천은 '찾을 수 있나'만 보면 되므로 **1페이지(50위)·페이지 대기 0** 으로 낮춰 돈다(순위추적 기본 6페이지는 너무 느리다). 결과는 6시간 캐시, 차단(blocked)된 응답은 캐시하지 않고 화면에 사유를 띄운다.
  - ⚠️ **처음엔 통합검색 HTML 에 placeId 가 있는지로 판정했다가 갈아엎었다(같은 날)** — 네이버 SERP 의 블로그 썸네일 데이터에 `"gdid":"blog_…","sid":"<placeId>"` 형태로 같은 숫자가 실려 있어 **플레이스가 안 보이는 키워드도 노출로 잡혔다**(사용자가 수동 검색으로 발견). 실제로 `초량미용실` 은 그 방식에서 '미노출' 이었지만 플레이스 순위로는 **8위**였다. 문자열 포함 검사로 노출을 판정하지 말 것.
  - 실측(단장헤어·부산 동구 초량동): 후보 14개 중 9개 채택 — 초량단장헤어 1위 · 부산단장헤어 1위 · 초량미용실 8위 · 부산동구미용실 18위 · 단장헤어 31위 등. 제외: 동구미용실·부산미용실·초량유명미용실.
