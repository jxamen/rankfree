...
2. **hub:publish** ([HubPublish](../app/Console/Commands/HubPublish.php) · [KeywordHubPublisher](../app/Domain/Keyword/KeywordHubPublisher.php)) — 승인 후보를 검색량 큰 순으로 상한(기본 10)만큼 발행: `KeywordReportBuilder::build()` → `KeywordSearch(origin=hub, user_id=NULL, category_id, snapshot, refreshed_at)`. **thin content 방지**: has_volume 없으면 발행하지 않고 rejected + '데이터 부족' 사유
   ...
   - **대량 수집 대기열·확장 pacing**(2026-07-24): 대기열은 `queueByCategory`([ExtKeywordShopSerpController](../app/Http/Controllers/Api/ExtKeywordShopSerpController.php)) — 1차 분류 라운드 로빈(1개마다 분류 교대)·미수집 우선·검색량순·5분 lease(중복 발급 방지). **제외 분류**는 `config('rankfree.hub.shop_exclude_roots')`(기본 `도서` — 쇼핑 SERP 구조가 달라 판매량·매출 산식 부적합, env `HUB_SHOP_EXCLUDE_ROOTS`)로 캐시 뒤 필터(config 즉시 반영).
     - 확장 `bulkShopStart`(background.js): **동시 수집 수량**은 관리자 화면 입력(기본 4·1~10, [index.blade.php](../resources/views/admin/keyword-hub/index.blade.php)) → warmStart 2에서 램프업(연속 성공 4회당 +1). **탭 오픈 스태거는 적응형**(v0.3.14) `max(1초, gap/liveConc)` — 콜드스타트(gap 6초·동시 2)엔 3초 간격으로 부드럽게, 성공이 이어질 때만 1.2초까지 좁히고, 차단이 뜨면 gap↑·liveConc↓ 로 자동 확대.
     - **동시수 sticky ceiling**(v0.3.15) — 램프업은 목표(conc)가 아니라 학습된 상한 `ceiling` 까지만. 429가 뜬 동시수보다 `ceiling` 을 낮춰 잡아, '5까지 올라감→429→2로 줆→다시 5→429' 진동을 끊는다. 몇 번 부딪히면 지속 가능한 낮은 동시수(예: 2)에서 안정되고, 성공 40회 무차단이면 `ceiling` 을 1만 재탐색. 상태줄 '동시 N/M' = 현재/상한. (실측: 부드럽게 열다 5까지 refill을 몰아 열어 반복 차단되던 문제 해소)
     - **서비스워커 사망 자동 재개 워치독**(v0.3.18, 2026-07-25) — "돌려두면 잘 수집하다가 자꾸 꺼짐"의 정체는 **MV3 서비스워커 유휴 종료**로 `bulkShopStart` 의 장기 async 루프가 통째로 죽는 것(예전엔 재기동 시 running 플래그만 내리고 자동 재개가 없어 그대로 멈췄다). → manifest 에 `alarms` 권한 추가, 시작 시 `rfBulk.desired=true`·원본 `params`(limit·delayMs·concurrency) 저장 + `chrome.alarms('rfBulkWatchdog', 0.5분)` 등록. 알람은 SW 종료·브라우저 재시작을 넘어 살아남아, **desired 인 동안 루프가 죽었으면(이 SW 에 살아있는 루프 없음·heartbeat 끊김) 원본 파라미터로 재개**한다(진행 `done`/`failed` 시드 보존 — 0 리셋 안 됨, 서버 큐가 이어 수집). 중복 시작 방지는 동기 선점 플래그 `bulkLoopActive`(첫 await 전 세움) + `heartbeat` 신선도 판정. `bulkShopStop`·정상 완주(finally)는 `desired=false` + 알람 정리로 재개를 끊는다. SW 재시작 훅은 desired 세션이면 running 을 '종료'로 깜빡이지 않고 즉시 재개 예약. 검증: 실확장 MV3 로드 + 실 `chrome.alarms` E2E 18/18(권한·시작 상태·중복거부·살아있는 루프 no-op·죽은 세션 재개+수치보존·중단 정리).
     - **차단 오탐 제거 + 진짜 429 백오프**(v0.3.13→v0.3.14): 동시 수집 시 크롬이 background 탭의 document_idle content script 주입을 미뤄 시작 신호가 늦게 와, 페이지가 3~4초에 떠도 옛 12초 alive 타임아웃이 '안 열림=차단'으로 오판했다. → 열림 판정을 `tabs.onUpdated status:complete`(로드 완료, 주입 무관)로 변경, alive 30초(둘 다 없을 때만 미로딩 차단), 캡차/nid 리다이렉트는 즉시 차단.
       - **진짜 429 처리**(v0.3.14): 네이버 429는 크롬 에러 페이지('HTTP ERROR 429' 텍스트)로 떠 `status:complete`는 나지만 content script 미주입 → 시작 신호가 영영 안 온다. complete 후 15초 내 시작 신호 없으면 이 429로 보고 **차단 처리**(함대 일시정지·rest·동시수 축소). v0.3.13 은 이걸 잠깐 '비차단'으로 뒀다가 진짜 429가 백오프를 안 걸고 계속 새 탭을 여는 문제가 생겨 되돌림. 정상 페이지는 complete→started 간격이 짧아 15초 오탐 없음.
