...
2. **hub:publish** ([HubPublish](../app/Console/Commands/HubPublish.php) · [KeywordHubPublisher](../app/Domain/Keyword/KeywordHubPublisher.php)) — 승인 후보를 검색량 큰 순으로 상한(기본 10)만큼 발행: `KeywordReportBuilder::build()` → `KeywordSearch(origin=hub, user_id=NULL, category_id, snapshot, refreshed_at)`. **thin content 방지**: has_volume 없으면 발행하지 않고 rejected + '데이터 부족' 사유
   ...
   - **대량 수집 대기열·확장 pacing**(2026-07-24): 대기열은 `queueByCategory`([ExtKeywordShopSerpController](../app/Http/Controllers/Api/ExtKeywordShopSerpController.php)) — 1차 분류 라운드 로빈(1개마다 분류 교대)·미수집 우선·검색량순·5분 lease(중복 발급 방지). **제외 분류**는 `config('rankfree.hub.shop_exclude_roots')`(기본 `도서` — 쇼핑 SERP 구조가 달라 판매량·매출 산식 부적합, env `HUB_SHOP_EXCLUDE_ROOTS`)로 캐시 뒤 필터(config 즉시 반영).
     - 확장 `bulkShopStart`(background.js): **동시 수집 수량**은 관리자 화면 입력(기본 4·1~10, [index.blade.php](../resources/views/admin/keyword-hub/index.blade.php)) → warmStart 2에서 목표까지 램프업(연속 성공 3회당 +1), 차단 시 절반. **탭 오픈 스태거** `max(1초, delayMs/conc)` 로 벌려 콜드스타트 429 방지(집계 오픈율 ≈ conc개/6초).
     - **차단 오탐 제거**(v0.3.13): 동시 수집 시 크롬이 background 탭의 document_idle content script 주입을 미뤄 시작 신호가 늦게 와, 페이지가 3~4초에 떠도 옛 12초 alive 타임아웃이 '안 열림=차단'으로 오판했다. → 열림 판정을 `tabs.onUpdated status:complete`(로드 완료, 주입 무관)로 변경, alive 30초(둘 다 없을 때만 미로딩 차단), 캡차/nid 리다이렉트는 즉시 차단, complete 후 시작신호 30초 미수신(에러페이지)은 **비차단 단순 실패**(함대 백오프 미유발). 검증: 실확장 SW 직접호출 결정적 4/4(지연주입 성공·캡차 즉시·미로딩 차단·로드후무응답 비차단).
