/**
 * RankFree 확장 — **사내 전용 오버레이**.
 *
 * ⚠️ 이 파일은 웹스토어 공개 배포본에 들어가지 않는다. extension/ 바깥에 두는 이유가 그것이다
 *    (게시 경로 두 곳 — scripts/publish-extension.mjs 와 app/Support/CwsPublisher.php — 이
 *     extension/ 디렉터리를 통째로 담기 때문에, 제외 목록 방식은 한쪽만 빠뜨려도 곧바로 유출된다).
 *
 * 담고 있는 것: 서버가 배정한 순위·키워드 수집을 사용자 조작 없이 수행하는 백그라운드 워커와,
 * 관리자 화면이 쓰는 대량 수집·SERP 대행 핸들러. 사용자가 시작하지 않는 동작이라
 * 크롬 웹스토어 단일 목적 정책에 걸린다 — 공개본에서 들어냈다(2026-08-10).
 *
 * 로드 방식: 사내 빌드에서 background.js 끝에 importScripts('internal.js') 한 줄이 덧붙는다.
 * **반드시 파일 끝**이어야 한다 — background.js 의 `const handlers` 는 TDZ 라
 * 선언 이전에 importScripts 를 부르면 ReferenceError 로 서비스워커가 죽는다.
 */
...
// ── 핸들러 확장 — background.js 의 handlers 객체에 사내 전용 핸들러를 덧붙인다 ──────────
...
// ── 알람·부트 훅 — 핸들러 등록 뒤에 와야 첫 알람이 빈 handlers 를 만나지 않는다 ─────────
try {
