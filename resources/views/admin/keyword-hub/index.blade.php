...
<div class="card p-5 mb-5">
    <div class="flex items-center justify-between gap-3 flex-wrap">
        <div>
            ...
            <div class="text-muted-soft mt-1" style="font-size:var(--fs-xs);">'쇼핑만 시작' 또는 '동시 시작'을 누르면 확장이 대기 키워드의 시장분석(판매량·매출)을 자동 수집하고, 수집되는 대로 발행이 따라갑니다. 브라우저를 켜둔 채 두세요(확장 v0.3.18+ 로그인 필요). 동시 수집 수량은 우측에서 조절합니다 — 차단 방지를 위해 2개로 시작해 목표까지 자동으로 올라갑니다.</div>
        </div>
...
(function () {
    ...
    function statLine() {
        ...
        if (lastStat.serp_10m > 0 && lastStat.market_10m === 0) {
            s += ' — ⚠️ 수집은 되는데 시장분석이 안 생깁니다: 확장이 구버전입니다. chrome://extensions 에서 v0.3.18 로 새로고침하세요';
        }
        return s;
    }
    function live() {
    ...
    function startCollect() {
        if (!hasExt()) {
            ...
            msg.textContent = '랭크프리 확장이 이 페이지에 연결되지 않았습니다 — chrome://extensions 에서 확장 새로고침(v0.3.18)·로그인 후 이 페이지를 새로고침하세요.';
            return false;
        }
        msg.style.color = '';
