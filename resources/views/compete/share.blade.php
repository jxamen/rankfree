...
<div style="max-width:1200px;margin:0 auto;padding:20px 16px 60px;">
    ...
    <div class="flex items-center gap-2 mb-6" style="height:48px;">
        ...
        <span class="text-muted-soft" style="font-size:12px;">경쟁 분석 공유 리포트</span>
    </div>
    ...
    @unless ($ymd)
        <div class="card p-8 text-center"><p class="text-muted" style="font-size:14px;">아직 분석 데이터가 없습니다.</p></div>
    @else
        {{-- KPI --}}
                ...
                <tbody>
                    @foreach ($rows as $r)
                        <tr style="border-top:1px solid var(--color-hairline-soft);{{ $r->is_mine ? 'background:color-mix(in srgb,var(--color-primary) 5%,var(--color-canvas));' : '' }}">
                            ...
                            @endforeach
                            @for ($k = 0; $k < 4; $k++)
                                <td class="text-center px-1 py-2" style="font-size:12px;@if($k===0)border-left:1px solid var(--color-hairline-soft);@endif">@if($r->wv && $r->wv[$k] > 0)<span class="text-ink">{{ $r->wv[$k] }}</span>@else<span class="text-muted-soft">·</span>@endif</td>
                            @endfor
                            <td class="text-center px-1 py-2 text-muted" style="font-size:12px;">{{ $r->visitor !== null ? number_format($r->visitor) : '—' }}</td>
                            @for ($k = 0; $k < 4; $k++)
                                <td class="text-center px-1 py-2" style="font-size:12px;@if($k===0)border-left:1px solid var(--color-hairline-soft);@endif">@if($r->wb && $r->wb[$k] > 0)<span class="text-ink">{{ $r->wb[$k] }}</span>@else<span class="text-muted-soft">·</span>@endif</td>
                            @endfor
        ...
        <p class="text-muted-soft mb-6" style="font-size:11px;">N1 유사도·N2 관련성·N3 랭킹 및 세부지표는 관측 신호 기반 <b>자체 추정치</b>이며 네이버 공식 점수가 아닙니다. 일별 순위 색: <span style="color:var(--color-primary);">상승</span>/<span style="color:var(--color-error);">하락</span>.</p>
