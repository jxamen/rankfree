...
<p class="text-muted mb-4" style="font-size:var(--fs-xs);">3일 연속 <b>{{ number_format((int) config('rankfree.shopping.track_depth', 400)) }}위 밖(미노출)</b>이면 순위체크가 자동 중단됩니다 · 목록 [재개]로 복구</p>
{{-- 액션(좌) + 기간·키워드 검색(우) — 카드 --}}
...
<p class="text-muted-soft mt-3" style="font-size:var(--fs-xs);">
    ...
    순위는 하루 1회 기록(당일 재확인 시 갱신)되며 최대 {{ number_format((int) config('rankfree.shopping.track_depth', 400)) }}위까지 조회합니다.
</p>
