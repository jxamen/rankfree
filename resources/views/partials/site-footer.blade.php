{{-- 다크 푸터 — 페이지를 닫는 다크 서피스. 링크는 실제 기능 카테고리에 맞춤 --}}
@php
    // 메뉴관리(사이트 SEO)에서 상태를 끈 라우트는 푸터 링크에서도 숨긴다(사이트 헤더와 동일 규칙).
    try {
        $__siteOff = \App\Models\Menu::where('area', 'site')->where('is_active', false)
            ->pluck('route')->map(fn ($r) => explode('?', trim((string) $r), 2)[0])->filter()->all();
    } catch (\Throwable $e) {
        $__siteOff = [];
    }
    $menuOn = fn (string $route) => ! in_array($route, $__siteOff, true);
@endphp
<footer class="bg-surface-dark text-on-dark-soft mt-auto">
    <div class="container-page" style="padding-top:64px;padding-bottom:48px;">
        <div class="grid gap-10 md:grid-cols-[1.5fr_1fr_1fr_1fr_1fr]">
            {{-- 브랜드 --}}
            <div>
                <div class="flex items-center gap-2 mb-3">
                    <span class="inline-flex items-center justify-center w-7 h-7 rounded-md bg-canvas text-ink font-display" style="font-size:var(--fs-sm);">R</span>
                    <span class="font-display text-on-dark" style="font-size:var(--fs-lg);">랭크프리</span>
                </div>
                <p style="font-size:var(--fs-xs);line-height:1.6;max-width:280px;">플레이스·쇼핑·키워드·블로그 순위와 경쟁사를 무료로 분석하고, 필요한 마케팅까지 한 곳에서.</p>
            </div>

            {{-- 플레이스 --}}
            <div>
                <div class="text-on-dark font-semibold mb-3" style="font-size:var(--fs-xs);">플레이스</div>
                <ul class="flex flex-col gap-2" style="font-size:var(--fs-xs);">
                    <li><a href="/#place" class="hover:text-on-dark transition">순위 추적</a></li>
                    <li><a href="/#place" class="hover:text-on-dark transition">경쟁 분석</a></li>
                    <li><a href="/#place" class="hover:text-on-dark transition">스마트플레이스 리포트</a></li>
                    @if ($menuOn('rank.check'))<li><a href="/rank-check" class="hover:text-on-dark transition">무료 순위체크</a></li>@endif
                </ul>
            </div>

            {{-- 쇼핑 --}}
            <div>
                <div class="text-on-dark font-semibold mb-3" style="font-size:var(--fs-xs);">쇼핑</div>
                <ul class="flex flex-col gap-2" style="font-size:var(--fs-xs);">
                    <li><a href="/#shopping" class="hover:text-on-dark transition">쇼핑 순위추적</a></li>
                    <li><a href="/#shopping" class="hover:text-on-dark transition">쇼핑 시장 분석</a></li>
                    <li><a href="/#shopping" class="hover:text-on-dark transition">셀러력 · 상품 SEO</a></li>
                    <li><a href="/#shopping" class="hover:text-on-dark transition">상품 리뷰 분석</a></li>
                </ul>
            </div>

            {{-- 키워드·블로그·API --}}
            <div>
                <div class="text-on-dark font-semibold mb-3" style="font-size:var(--fs-xs);">키워드 · 블로그</div>
                <ul class="flex flex-col gap-2" style="font-size:var(--fs-xs);">
                    <li><a href="/#keyword" class="hover:text-on-dark transition">키워드 분석</a></li>
                    <li><a href="/#keyword" class="hover:text-on-dark transition">키워드 추천 · 대량</a></li>
                    <li><a href="/#blog" class="hover:text-on-dark transition">블로그 지수 분석</a></li>
                    @if ($menuOn('developers'))<li><a href="/console/developers" class="hover:text-on-dark transition">순위 API</a></li>@endif
                </ul>
            </div>

            {{-- 회사 --}}
            <div>
                <div class="text-on-dark font-semibold mb-3" style="font-size:var(--fs-xs);">회사</div>
                <ul class="flex flex-col gap-2" style="font-size:var(--fs-xs);">
                    <li><a href="/#marketing" class="hover:text-on-dark transition">마케팅 서비스</a></li>
                    <li><a href="/#pricing" class="hover:text-on-dark transition">요금</a></li>
                    <li><a href="/support" class="hover:text-on-dark transition">고객지원</a></li>
                    <li><a href="/terms" class="hover:text-on-dark transition">이용약관</a></li>
                    <li><a href="/privacy" class="hover:text-on-dark transition">개인정보처리방침</a></li>
                </ul>
            </div>
        </div>

        <div class="border-t border-white/10 mt-10 pt-6" style="font-size:var(--fs-xs);color:var(--color-muted-soft);">
            <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
                {{-- 사업자 정보 --}}
                <div style="line-height:1.9;">
                    <div class="text-on-dark" style="font-weight:600;">(주)제이커브인터렉티브</div>
                    <div>사업자등록번호 : 753-88-01488 &nbsp;|&nbsp; 대표이사 : 김태훈</div>
                    <div>주소 : 서울특별시 금천구 가산디지털1로, 가산더블유센터 703호</div>
                    <div><a href="tel:0216682612" class="hover:text-on-dark transition">02-1668-2612</a> &nbsp;·&nbsp; <a href="mailto:jcurve19@gmail.com" class="hover:text-on-dark transition">jcurve19@gmail.com</a></div>
                </div>
                {{-- SNS (채널 개설 시 href 교체) --}}
                <div class="flex items-center gap-2 shrink-0">
                    <a href="#" target="_blank" rel="noopener" aria-label="블로그" title="블로그"
                       class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 text-on-dark-soft hover:text-on-dark transition">
                        <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24" aria-hidden="true"><path d="M16.273 12.845 7.376 0H0v24h7.727V11.155L16.624 24H24V0h-7.727z"/></svg>
                    </a>
                    <a href="#" target="_blank" rel="noopener" aria-label="인스타그램" title="인스타그램"
                       class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 text-on-dark-soft hover:text-on-dark transition">
                        <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
                    </a>
                    <a href="#" target="_blank" rel="noopener" aria-label="유튜브" title="유튜브"
                       class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 text-on-dark-soft hover:text-on-dark transition">
                        <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24" aria-hidden="true"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                    </a>
                    <a href="#" target="_blank" rel="noopener" aria-label="카카오톡 채널" title="카카오톡 채널"
                       class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 text-on-dark-soft hover:text-on-dark transition">
                        <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3c-5.799 0-10.5 3.664-10.5 8.185 0 2.881 1.912 5.409 4.788 6.868-.211.783-.763 2.844-.873 3.284-.136.546.2.539.42.392.173-.115 2.755-1.872 3.869-2.63.74.11 1.507.166 2.296.166 5.799 0 10.5-3.664 10.5-8.185C22.5 6.664 17.799 3 12 3"/></svg>
                    </a>
                </div>
            </div>
            <div class="mt-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <span>© {{ date('Y') }} 랭크프리. All rights reserved.</span>
                <span>순위·점수는 자체 추정치이며 공식 지표가 아닙니다.</span>
            </div>
        </div>
    </div>
</footer>
