...
<section class="container-page py-10 lg:py-14" style="max-width:720px;padding-left:0;padding-right:0;">
    ...
    <form method="POST" action="{{ route('community.update', $post) }}" class="card p-6">
        @csrf @method('PUT')
        <div class="mb-4">
            ...
            <select name="category_id" class="input" required>
                @foreach ($categories as $cat)
                    <option value="{{ $cat->id }}" @selected(old('category_id', $post->category_id) == $cat->id)>{{ $cat->icon }} {{ $cat->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-4">
        ...
        <div class="mb-5">
            ...
            <textarea name="body" class="input" style="height:220px;padding:12px 14px;line-height:1.7;" maxlength="20000" placeholder="내용을 입력하세요" required>{{ old('body', $post->body) }}</textarea>
        </div>
