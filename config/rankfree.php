...
return [
    /*
    |--------------------------------------------------------------------------
    | 관리자 전용 호스트(2026-07-24) — 어드민 URL 은닉
    |--------------------------------------------------------------------------
    | 설정하면 /admin/* 은 이 호스트로 접근할 때만 열리고, 다른 호스트(rankfree.kr 포함)에선
    | 전부 404(존재 은닉). 와일드카드 *.rankfree.co.kr 인프라로 서브도메인 무작업 적용.
    | 예) ADMIN_HOST=ops-xxxx.rankfree.co.kr — 비우면 게이트 없음(로컬 기본).
    */
    'admin_host' => env('ADMIN_HOST', ''),

    /*
    |--------------------------------------------------------------------------
    | 대표 도메인 통합(2026-07-27)
    |--------------------------------------------------------------------------
    | redirect_hosts 로 들어온 요청을 host 로 301 보낸다(경로·쿼리 유지).
    | rankfree.co.kr 과 rankfree.kr 이 같은 내용을 각자 200 + self-canonical 로 서빙해
    | 검색 자산이 갈라지던 문제 해결.
    |
    | ⚠️ **정확히 일치하는 호스트만** 넣는다(서브도메인 금지) —
    |    ops-….rankfree.co.kr(어드민)·sunny-….rankfree.co.kr(단축 URL, 이미 배포된 주소)이
    |    같은 상위 도메인을 쓰므로 와일드카드로 처리하면 관리자 접속과 단축 URL 이 전부 깨진다.
    */
    'canonical' => [
        'host' => env('CANONICAL_HOST', 'rankfree.kr'),
        'redirect_hosts' => array_values(array_filter(array_map(
            'trim',
            explode(',', (string) env('CANONICAL_REDIRECT_HOSTS', 'rankfree.co.kr,www.rankfree.co.kr')),
        ))),
    ],
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | 방문 집계 제외 IP — 이 IP 들에는 커스텀 헤드(GA4 gtag 등 추적 코드)를 출력하지 않는다
    |--------------------------------------------------------------------------
    | 사무실·내부 IP 를 콤마로 나열: ANALYTICS_EXCLUDE_IPS=1.2.3.4,5.6.7.8
    | 이후 수집분부터 제외(이미 쌓인 데이터는 소급 불가). 정석은 GA4 관리 > 내부 트래픽 필터.
    */
    'analytics_exclude_ips' => array_values(array_filter(array_map('trim', explode(',', (string) env('ANALYTICS_EXCLUDE_IPS', ''))))),
